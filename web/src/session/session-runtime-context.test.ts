...
describe("session runtime context", () => {
  ...
  it("resets the runtime without installing window compatibility shims", () => {
    ...
    expect(windowKeys).not.toContain("__piSessionDataModel");
    expect(windowKeys).not.toContain("navigateTo");
    expect(windowKeys).not.toContain("__piReconcileEntries");
    expect(windowKeys).not.toContain("__piContentRuntime");
