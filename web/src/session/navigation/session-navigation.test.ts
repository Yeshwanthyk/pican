...
const runImmediate = (fn: () => void, delay = 0): void => {
  if (!delay) fn();
};
...
describe("session navigator (nav + scroll)", () => {
  ...
  it("scrolls to the bottom in bottom mode", () => {
    ...
    if (content) Object.defineProperty(content, "scrollHeight", { value: 500, configurable: true });
  ...
  it("opens collapsed ancestor details before scrolling to a nested entry", () => {
    ...
    expect(group instanceof HTMLDetailsElement ? group.open : undefined).toBe(true);
    expect(toolFold instanceof HTMLDetailsElement ? toolFold.open : undefined).toBe(true);
