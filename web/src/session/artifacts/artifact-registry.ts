...
type ArtifactKind = "preview" | "code";
...
interface SessionMessage {
  ...
  readonly content?: string | ReadonlyArray<ToolCall>;
  readonly toolCallId?: string;
  readonly isError?: boolean;
...
type FileOperation =
  | { readonly op: "rm"; readonly path: string }
  | { readonly op: "mv"; readonly from: string; readonly to: string };
...
function previewTypeFor(lang: string): string {
...
function applyEdits(content: string, edits: unknown): string {
  ...
  for (const e of edits) {
    if (!isUnknownRecord(e)) continue;
    const edit: EditOperation = e;
...
function tokenizeShell(s: string): string[] | null {
  const tokens: string[] = [];
  let cur = "";
  let quote = "";
  let started = false;
 ...
 */
export function parseFileOps(command: unknown): FileOperation[] {
  ...
  const source = srcs[0];
  if (source !== undefined && dst !== undefined && srcs.length === 1) {
    return [{ op: "mv", from: source, to: dst }];
  }
  return []; // multiple sources without a directory dst — ambiguous, skip
}

function applyBashOps(command: unknown, byPath: Map<string, ArtifactDescriptor>): void {
...
function indexToolResults(entries: ReadonlyArray<SessionEntry>): Map<string, ToolResult> {
  const map = new Map<string, ToolResult>();
  for (const entry of entries) {
    if (entry && entry.type === "message" && entry.message && entry.message.role === "toolResult") {
      if (entry.message.toolCallId) map.set(entry.message.toolCallId, entry.message);
    }
  }
  return map;
}
...
function applyFileToolCall(
  call: ToolCall,
  entry: SessionEntry,
  order: ArtifactDescriptor[],
  byPath: Map<string, ArtifactDescriptor>,
 ...
 */
function* fencedBlocks(text: string): Generator<{ lang: string; content: string }> {
...
function* artifactsFromText(
  ...
  entry: SessionEntry,
  blockIndex: number,
  minCodeBlockLines: number,
): Generator<ArtifactDescriptor> {
  let fenceIndex = 0;
...
export function collectArtifacts(
  ...
  { minCodeBlockLines = 6 }: { readonly minCodeBlockLines?: number } = {},
): ArtifactDescriptor[] {
  if (!Array.isArray(entries)) return [];
  const order: ArtifactDescriptor[] = []; // file + snippet artifacts in first-seen order
  const byPath = new Map<string, ArtifactDescriptor>(); // current path -> file artifact
