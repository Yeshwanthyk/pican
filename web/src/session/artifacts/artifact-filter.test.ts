...
const file = (filePath: string) => ({ id: filePath, filePath, content: "x" });
const snippet = (id: string) => ({ id, filePath: null, content: "x", source: "fenced" });
...
describe("readArtifactSettings", () => {
  function fakeStorage(map: Readonly<Record<string, string>>) {
    return { getItem: (k: string) => map[k] ?? null };
  }
