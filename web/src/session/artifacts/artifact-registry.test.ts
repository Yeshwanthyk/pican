...
function assistant(id: string, content: ReadonlyArray<TestBlock>) {
  return { id, type: "message", message: { role: "assistant", content } };
}
function writeCall(callId: string, file_path: string, content: unknown): TestBlock {
...
function bashCall(callId: string, command: string): TestBlock {
  return { type: "toolCall", id: callId, name: "bash", arguments: { command } };
}
function toolResult(callId: string, isError = false) {
  return {
    id: `r-${callId}`,
    type: "message",
...
function text(t: string): TestBlock {
