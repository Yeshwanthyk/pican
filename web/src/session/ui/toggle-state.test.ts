...
function decodeStored(value: string | null): unknown {
  return value === null
    ? undefined
    : runSync(Schema.decodeUnknownEffect(Schema.UnknownFromJsonString)(value));
}

function styleOf(document: Document, selector: string): CSSStyleDeclaration | undefined {
...
describe("toggle state helpers", () => {
  ...
  it("saves state under the session id; no-ops without one", () => {
    ...
    expect(decodeStored(storage.getItem(TOGGLE_STATE_STORAGE_KEY))).toEqual({
      "sess-a": {
        thinkingExpanded: false,
        toolsVisible: true,
        toolOutputsExpanded: true,
  ...
  it("hides the tool-call placeholder when tools are visible", () => {
    ...
    expect(styleOf(dom.window.document, ".tool-call-collapsed")?.display).toBe("none");
  ...
  it("controller persists toggles under its session id and other sessions are unaffected", () => {
    ...
    dom.window.document
      .querySelector<HTMLButtonElement>('[data-action="toggle-thinking"]')
      ?.click();
