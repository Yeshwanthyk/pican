...
describe("session search/filter UI", () => {
  it("wires search input and filter buttons", () => {
    ...
    const input = jsdom.window.document.querySelector<HTMLInputElement>("#tree-search");
    if (!input) return;
    input.value = "needle";
    ...
    jsdom.window.document.querySelector<HTMLButtonElement>('[data-filter="all"]')?.click();
