...
describe('chat api helpers', () => {
  ...
  it('wraps chat endpoints', async () => {
    ...
    await listModels('s.jsonl', { fetchImpl });
    ...
    expect(fetchImpl).toHaveBeenNthCalledWith(4, '/api/models?id=s.jsonl');
  ...
  it('retains the merged model endpoint for global callers', async () => {
    const fetchImpl = vi.fn(() => Promise.resolve(new Response('{}')));
