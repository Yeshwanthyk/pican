...
describe("ShareDialog", () => {
  it("starts hidden and shows the gist + preview URLs after a successful share", async () => {
    ...
    await tick();
    expect(document.querySelector<HTMLElement>("#share-overlay")?.style.display).toBe("none");

    await fireEvent.click(btn);
    expect(fetch).toHaveBeenCalledWith("/share?id=s.jsonl", {
      method: "POST",
    ...
    await waitFor(() => {
      expect(document.querySelector<HTMLElement>("#share-overlay")?.style.display).toBe("");
    });
    expect(document.querySelector<HTMLInputElement>("#share-gist-url")?.value).toBe(
      "https://gist.example/abc",
    );
    expect(document.querySelector<HTMLInputElement>("#share-preview-url")?.value).toBe(
