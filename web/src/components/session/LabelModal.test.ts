...
describe("LabelModal", () => {
  it("saves the typed label and closes", async () => {
    ...
    await tick();
    const input = document.querySelector<HTMLInputElement>("#label-modal-input");
    if (input) input.value = "Checkpoint";
    input?.dispatchEvent(new Event("input", { bubbles: true }));
    await tick();
    document.querySelector<HTMLButtonElement>(".label-modal-save")?.click();
    await tick();
  ...
  it("shows remove for existing labels and clears the label", async () => {
    ...
    await tick();
    const remove = document.querySelector<HTMLButtonElement>(".label-modal-remove");
  ...
  it("hides remove when there is no existing label", async () => {
    ...
    await tick();
    expect(document.querySelector<HTMLButtonElement>(".label-modal-remove")?.hidden).toBe(true);
  });
});
