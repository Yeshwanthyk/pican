...
describe("setupMentionAutocomplete controller", () => {
  function setup(files = SAMPLE) {
    ...
    const getFiles = vi.fn(
      (_sessionId: string, _query: string, _options: { readonly signal?: AbortSignal }) =>
    ...
    return {
      ...
      popup:
        document.querySelector<HTMLElement>("#pi-chat-mention-popup") ??
  ...
  it("keeps the @ and popup open when a directory is selected", async () => {
    ...
    type(textarea, "@lib");
    await flush();
    document.querySelector<HTMLButtonElement>(".slash-item")?.click();
