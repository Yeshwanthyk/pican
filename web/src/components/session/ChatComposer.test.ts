...
function getById(
  dom: JSDOM,
  id: "pi-chat-cancel" | "pi-chat-send" | "pi-chat-expand",
): HTMLButtonElement;
function getById(dom: JSDOM, id: string): HTMLElement;
function getById(dom: JSDOM, id: string): HTMLElement {
...
function queryRequired<T extends Element = HTMLElement>(dom: JSDOM, selector: string): T {
  const element = dom.window.document.querySelector<T>(selector);
  assert(element, `missing ${selector}`);
  return element;
}

function childRequired<T extends Element = HTMLElement>(root: Element, selector: string): T {
...
describe("chat composer runner", () => {
  ...
  it("marks unavailable composer", () => {
    ...
    expect(getById(dom, "pi-chat-composer").title).toBe("no cwd");
  });
  ...
  it("passes escapeHtml into model selector setup", () => {
    ...
    dom.window.document.dispatchEvent(new dom.window.Event("DOMContentLoaded"));
    const setupOptions = setupModelSelector.mock.calls.at(0)?.at(0);
    assert(setupOptions);
    expect(setupOptions.escapeHtml).toBe(escapeHtml);
  });
  ...
  it("cancels chat and refreshes worker status without throwing", async () => {
    ...
    getById(dom, "pi-chat-cancel").click();
    await new Promise((r) => setTimeout(r, 0));
    await new Promise((r) => setTimeout(r, 0));
  ...
  it("toggles composer expanded state and persists per session", () => {
    ...
    const shell = queryRequired(dom, ".pi-chat-shell");
    const btn = getById(dom, "pi-chat-expand");
  ...
  it("restores composer expanded state from localStorage", () => {
    ...
    const storage = new Map<string, string>([["pican:chat:composer-expanded:abc", "1"]]);
    Object.defineProperty(dom.window, "localStorage", {
      configurable: true,
      value: {
        getItem: (k: string) => storage.get(k) ?? null,
  ...
  it("attaches pasted image from clipboard", () => {
    ...
    const attachment = childRequired(getById(dom, "pi-chat-attachments"), ":scope > *");
  ...
  it("ignores non-image paste", () => {
    ...
    expect(getById(dom, "pi-chat-attachments").children.length).toBe(0);
  ...
  it("deduplicates pasted images", () => {
    ...
    expect(getById(dom, "pi-chat-attachments").children.length).toBe(1);
  });
  ...
  it("adds a text attachment from pi-chat-attach-text and folds it into the sent message", async () => {
    ...
    const chip = queryRequired<HTMLButtonElement>(dom, ".pi-chat-attachment-text");
    expect(getById(dom, "pi-chat-send").disabled).toBe(false);

    // Clicking the chip opens the viewer showing the original selection.
    chip.click();
    expect(getById(dom, "pi-chat-attachment-modal").hidden).toBe(false);
    expect(queryRequired(dom, ".pi-chat-attachment-card-quote").textContent).toBe("hello world");
    queryRequired<HTMLButtonElement>(dom, '[data-action="close-attachment"]').click();
    expect(getById(dom, "pi-chat-attachment-modal").hidden).toBe(true);

    // Submitting folds the selection (as a blockquote + note) before the typed text.
    getById(dom, "pi-chat-message").value = "please fix";
    ...
    const firstSend = sendChat.mock.calls.at(0);
    assert(firstSend);
    const sentBody = firstSend[1];
    expect(sentBody.get("message")).toBe("> hello world\n\nrename this\n\nplease fix");
  ...
  it("does not focus the message textarea on mobile page load", () => {
    ...
    dom.window.matchMedia = vi.fn((query: string): MediaQueryList => {
      return {
    ...
    expect(queryRequired(dom, ".pi-chat-shell").classList).toContain("composer-collapsed");
  });
  ...
  it("Shift+Tab in the textarea cycles thinking level", () => {
    ...
    getById(dom, "pi-chat-message").dispatchEvent(event);
  ...
  it("updates circular context usage based on entries and active model", () => {
    ...
    const el = getById(dom, "pi-chat-context-usage");
    expect(el.style.display).not.toBe("none");
    const text = childRequired(el, ".pi-context-text");
    expect(text.textContent).toBe("8%"); // 10000 / 128000 = 7.8% => 8%

    const fill = childRequired(el, ".pi-context-fill");
  ...
  it("toggles detailed context usage popover and formats values correctly", () => {
    ...
    const el = getById(dom, "pi-chat-context-usage");
    const popover = getById(dom, "pi-chat-context-popover");
    ...
    expect(getById(dom, "pi-popover-val-input").textContent).toBe("11.2k");
    expect(getById(dom, "pi-popover-val-cache-read").textContent).toBe("5.4k");
    expect(getById(dom, "pi-popover-val-cache-write").textContent).toBe("0");
    expect(getById(dom, "pi-popover-val-output").textContent).toBe("1.0k");
    expect(getById(dom, "pi-popover-val-total").textContent).toBe("17.6k");

    expect(childRequired(popover, ".pi-popover-used").textContent).toBe("17.6k");
    expect(childRequired(popover, ".pi-popover-limit").textContent).toBe("1.0M"); // gemini-1.5-flash is 1M limit
  });
  ...
  it("uses last assistant totalTokens for context window %, not cumulative I/O", () => {
    ...
    el.click();
    expect(popover.style.display).toBe("block");
    expect(getById(dom, "pi-popover-val-input").textContent).toBe("1.5k"); // 1000+500
    expect(getById(dom, "pi-popover-val-output").textContent).toBe("800"); // 500+300
    expect(getById(dom, "pi-popover-val-cache-read").textContent).toBe("1.0k"); // 0+1000
    expect(getById(dom, "pi-popover-val-cache-write").textContent).toBe("1.0k"); // 1000+0
    expect(getById(dom, "pi-popover-val-total").textContent).toBe("4.3k"); // 1500+800+1000+1000

    // Popover hero shows contextTokens (last assistant, not cumulative)
    expect(childRequired(popover, ".pi-popover-used").textContent).toBe("1.8k");
    expect(childRequired(popover, ".pi-popover-limit").textContent).toBe("128k");
  });
  ...
  it("loads dynamic context limits from chatApi.listModels()", async () => {
    ...
    const popover = getById(dom, "pi-chat-context-popover");
    expect(childRequired(popover, ".pi-popover-limit").textContent).toBe("1.2M"); // 1,234,567 formats to 1.2M
  });
});
