...
type NoticeElement = HTMLDivElement & { _hideTimer?: TimerHandle };
...
export function showCwdToast(
  ...
  if (!composer) return;
  let notice = documentImpl.querySelector<NoticeElement>("#pi-chat-cwd-toast");
  if (!notice) {
    notice = documentImpl.createElement("div") as NoticeElement;
  ...
  if (notice._hideTimer !== undefined) clearTimeoutImpl(notice._hideTimer);
...
export function setupCwdCopy({
  ...
  readonly tImpl?: typeof t;
  readonly setTimeoutImpl?: typeof setTimeout;
  readonly clearTimeoutImpl?: typeof clearTimeout;
} = {}): void {
  const cwdEl = documentImpl.querySelector<HTMLElement>(".pi-chat-cwd");
  if (!cwdEl) return;
