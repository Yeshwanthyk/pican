...
function setup(opts: Parameters<typeof setupAskQuestionHandlers>[0]) {
...
describe("setupAskQuestionHandlers", () => {
  it("sends a single answer immediately when no submit is required", async () => {
    ...
    document.querySelector<HTMLButtonElement>(".ask-question-option-action")?.click();
    await tick();

    expect(sendChatMessage).toHaveBeenCalledWith('"Pick one" = "A"', []);
    expect(document.querySelector<HTMLButtonElement>(".ask-question-option-action")?.disabled).toBe(
      true,
    );
  });
  ...
  it("re-enables an immediate answer when send fails", async () => {
    ...
    const option = document.querySelector<HTMLButtonElement>(".ask-question-option-action");
    option?.click();
    await tick();
  ...
  it("collects multi-select answers and submits them together", async () => {
    ...
    const options = document.querySelectorAll<HTMLButtonElement>(".ask-question-option-action");
    options[0]?.click();
    options[1]?.click();
    expect(document.querySelector<HTMLElement>(".ask-question-actions")?.style.display).toBe("");
    document.querySelector<HTMLButtonElement>(".ask-question-submit-btn")?.click();
    await tick();
