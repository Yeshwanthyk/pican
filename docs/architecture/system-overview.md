...
`app.Config` and `app.Run(ctx, config) error` are the reusable hosting boundary. `Run` owns the listener, server, database, watchers, catalog loops, and workers it creates; cancellation shuts them down and returns. It does not install signal handlers or call `os.Exit`. `cmd/pican` is only the CLI/environment/version/signal adapter. The startup contract also accepts an optional browser-safe `HostNavigationURL`.

Standalone mode is the full local Pican product: its complete catalog, supported runtimes, and supported features retain the local `~/.pi/agent` shape.

Hosted mode is intentionally Codex-only and requires one canonical absolute `WorkspaceRoot`, one absolute `StateRoot` contained by that workspace, one normalized `BasePath` such as `/s/abc123`, proxy-only authentication, and an exact child environment supplied by the host. That mount is one fixed workspace containing many conversations, not a product bound to one conversation. The current shared UI still calls those conversations Sessions. The hosted product contract calls each conversation a **Thread** in user-facing copy; existing internal Session API routes, payloads, persisted formats, and symbols keep their current names. The current catalog can reconcile multiple contained native Codex threads, and `/api/new-session` can create additional contained Threads. Scotty bootstrap may create one initial Thread, but that does not define the hosted product boundary.

The server registers a root-based inner mux once, then the base-path handler strips the configured mount before dispatch. Requests outside the mount return `404`. The initial live HTML contains one inert application-context JSON object with only `mode` (`standalone` or `hosted`) and the optional host-navigation URL. Workspace and state roots, authentication values, child environment, credentials, and runtime/provider details are never copied into it. The frontend validates this object before mounting and falls back to standalone when it is missing or malformed.

Live HTML, hashed assets, styles, API requests, SSE, and navigation all derive from the same base-path value. Standalone additionally registers the icon, manifest, and service-worker routes and boots the service worker; hosted mode registers none of those PWA handlers and emits no PWA boot metadata. Static export/share remains a separate self-contained render and does not inherit live mount or network behavior.

Proxy-only mode accepts exactly one instance of the configured header and compares its value in constant time. It accepts no query token, login form, Bearer token, `X-Pican-Token`, Pican cookie, or browser fallback. The proxy token is read from `PICAN_PROXY_TOKEN`; there is deliberately no CLI token flag. It is not copied into the Codex environment, state, projections, responses, or logs. The embedding host must keep Pican off the public network, authenticate the browser, strip browser credentials, and inject the private header only on its internal hop.

Every hosted create, browse, file, git, task, project, and child-working-directory boundary resolves through one canonical symlink-aware workspace resolver. The root and descendants are accepted; raw `..`, siblings, scoped filesystem symlinks, task store/output symlinks, and external Git gitdirs are rejected before reads or creation. Catalog reconciliation validates each authoritative Codex thread cwd before projection, and direct API/bootstrap/share reads repeat the same check. Native unarchive validates archived metadata before mutation. The Codex child and hosted Git helpers receive only the configured allowlisted environment, including `CODEX_*`, `OPENAI_*`, `GH_*`, and `GITHUB_*` names. That filter is name-based: Scotty must supply only opaque sentinels under those names and must never pass raw provider credentials to Pican. Pican never resolves the sentinel values. Pican auth variables and unrelated host/provider secrets are stripped.

Hosted mode does not publish GitHub gists, run remote update checks, install or restart Pican, or invoke the ambient Pi auto-title model path. Share preview remains a local static render, and auto-title falls back to the local heuristic. Scotty owns execution-provider selection and provisioning, workspace sleep, workspace resume/restore, credentials, workspace deletion, external egress, and runtime replacement. Pican may manage individual contained Thread lifecycle, but it does not manage workspace lifecycle. Selection and `/session?id=...` deep links identify one Thread without binding the mounted product to it. See the [hosted workspace and multi-Thread research contract](../research/scotty-single-session-pican.md) for recommended responsibilities, UI, recovery, and acceptance criteria.
