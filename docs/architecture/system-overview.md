...
Hosted mode does not publish GitHub gists, run remote update checks, install or restart Pican, or invoke the ambient Pi auto-title model path. Share preview remains a local static render, and auto-title falls back to the local heuristic. The embedding host owns external egress and runtime replacement.
