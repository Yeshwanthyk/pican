...
`SessionsPage.svelte` owns the page shell and orchestrates Svelte components for the ticker-row session list, desktop right rail, mobile thumb bar, command palette, home menu, new-session modal, and tracked-project modal. Its URL-backed scopes are Projects (the default focused home), All Sessions, Archived, and one exact project detail. Subagents remains a separate global route; Workflows and Tasks are session-scoped actions in `CommandMenu.svelte`. `web/src/index/` contains pure data/API helpers (`sessions.ts`) for normalization, pinned/date/project grouping, view-scoped requests, and project curation.

Data comes from APIs such as `/api/sessions`, `/api/runtimes`, `/api/new-session`, `/api/projects`, `/api/archives`, `/api/recent-locations`, and `/events?id=__all__`. The default `view=home` response is already bounded on the server: every unarchived pinned session plus at most six sessions per explicitly tracked project. Unpinned sessions from untracked projects stay available through All Sessions instead of appearing in a separate live-activity group. The client keeps first-render order stable across soft activity refreshes, prepends genuinely new response members, and keeps pins in SQLite `pinOrder` even while they run or wait. Running and waiting change the status styling of pinned or tracked-project rows without moving them between home groups. The desktop home rail renders waiting sessions present in the bounded home response; configured peer machines remain a separate conditional section. All Sessions and exact-project views keep normal pagination; typed palette search deliberately uses `view=all`. Session summaries include the latest unresolved tool activity; an unresolved `ask_user_question` carries its first question and answer labels so the home rail can reply through the existing `/api/chat` path. Runtime plus native ID participate in search. The new-session modal selects among configured, currently available runtimes and sends the choice to `/api/new-session`; successful web creation tracks the persisted project path. Running-session status and curation changes are pushed through the shared SSE helpers.
...
Message-content copy is rendered by the shared `SessionEntry` component. The live app and static export inject their own clipboard adapters, keeping export free of the live Effect runtime bridge.
